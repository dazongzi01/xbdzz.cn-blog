...
💡 **小贴士：** 这样做是把 dbuser 这个用户跟项目绑定，更安全
...
2. **找到 system-admin 文件夹下的 application.yml**

   改成这样：
   ```yaml
   spring:
     datasource:
       url: jdbc:mysql://localhost:3306/shop_db?useUnicode=true&characterEncoding=utf8mb4&serverTimezone=UTC
       username: dbuser
       password: 123456
...
1. **打开 system-front 文件夹下的 application.yml**
...
2. 用户名和密码对不对？
   ```bash
   mysql -h localhost -u dbuser -p123456 -e "SELECT 1"
   ```
...
```bash
java -Xmx2G -Xms512M -jar system-admin.jar
```
