...
| 配置项 | 推荐值 | 说明 |
|--------|--------|------|
| Bucket 名称 | `crmeb-files` | 全局唯一，小写字母数字短横线 |
...
| Bucket 名称 | `crmeb-files` | Bucket 列表 |
...
| Bucket | `crmeb-files-1234567890` | 存储桶列表（含 APPID） |
...
oss:
  ...
  tencent:
    ...
    region: ap-beijing
    bucket-name: crmeb-files-1234567890
    domain: https://cdn.example.com  # 可选
...
{项目}-{环境}-{类型}
示例：
crmeb-prod-images     # 生产环境图片
crmeb-test-images     # 测试环境图片
crmeb-prod-files      # 生产环境文件
```
